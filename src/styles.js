...
.sidebar { width: 240px; flex-shrink: 0; background: var(--ink); color: #EDEAE2; display: flex; flex-direction: column; }
...
.sidebar__footer { display: flex; align-items: center; gap: 10px; padding: 14px 16px; border-top: 1px solid rgba(255,255,255,0.08); }
...
.pos-layout { display: grid; grid-template-columns: 1fr 360px; gap: 20px; align-items: start; height: 100%; }
...
.modal-overlay { position: fixed; inset: 0; background: rgba(32, 43, 59, 0.55); display: flex; align-items: center; justify-content: center; z-index: 50; padding: 20px; }
.modal-box { background: var(--surface); border-radius: 10px; width: 100%; max-height: 88vh; overflow-y: auto; box-shadow: 0 20px 50px rgba(0,0,0,0.25); }
...
@media (max-width: 980px) {
  .sidebar { width: 76px; }
  .brand__name, .brand__tag, .nav__group-label, .nav__item span, .sidebar__who { display: none; }
